...
for(j in c("EVMMOD", "EVSMOD", "EVLMOD", "EVHMOD")){
  ...
  if(is.na(file.info(paste(j, '3a.tif', sep=""))$size)){
    if(j=="EVSMOD"){
      system(paste(gdalwarp, ' mod13a3.vrt -t_srs \"+proj=longlat +datum=WGS84\" -ot Int16 ', j, '3a.tif', ' -r near -te -180 -90 180 90 -tr ', 1/120,' ', 1/120, sep=""))
    } else {
      system(paste(gdalwarp, ' mod13a3.vrt -t_srs \"+proj=longlat +datum=WGS84\" -ot Int16 -dstnodata \"-32767\" ', j, '3a.tif', ' -r near -te -180 -90 180 90 -tr ', 1/120,' ', 1/120, sep=""))   ##  
    }
  }
  # 2.5 km:
  if(is.na(file.info(paste(j, '2a.tif', sep=""))$size)){
    if(j=="EVSMOD"){
      system(paste(gdalwarp, ' mod13a3.vrt -t_srs \"+proj=longlat +datum=WGS84\" -ot Int16 ', j, '2a.tif', ' -r bilinear -te -180 -90 180 90 -tr ', 1/40,' ', 1/40, sep=""))
    } else {
      system(paste(gdalwarp, ' mod13a3.vrt -t_srs \"+proj=longlat +datum=WGS84\" -ot Int16 -dstnodata \"-32767\" ', j, '2a.tif', ' -r bilinear -te -180 -90 180 90 -tr ', 1/40,' ', 1/40, sep=""))
    }
  }
  # 5 km:
  if(is.na(file.info(paste(j, '1a.tif', sep=""))$size)){
    if(j=="EVSMOD"){
      system(paste(gdalwarp, ' mod13a3.vrt -t_srs \"+proj=longlat +datum=WGS84\" -ot Int16 ', j, '1a.tif', ' -r bilinear -te -180 -90 180 90 -tr ', 1/20,' ', 1/20, sep=""))
    } else {
      system(paste(gdalwarp, ' mod13a3.vrt -t_srs \"+proj=longlat +datum=WGS84\" -ot Int16 -dstnodata \"-32767\" ', j, '1a.tif', ' -r bilinear -te -180 -90 180 90 -tr ', 1/20,' ', 1/20, sep=""))
    }
  }
  ## 20 km:
  ## TH: This creates a line around equator!! I had to re-make these maps by hand on the end in SAGA GIS. 
  #if(j=="EVSMOD"){
  #  system(paste(gdalwarp, ' ', j, '1a.tif', ' -t_srs \"+proj=longlat +datum=WGS84\" -ot Int16 ', j, '0a.tif', ' -r bilinear -te -180 -90 180 90 -tr ', 1/5,' ', 1/5, sep=""))
  #} else {
  #  system(paste(gdalwarp, ' ', j, '1a.tif', ' -t_srs \"+proj=longlat +datum=WGS84\" -ot Int16 -dstnodata \"-32767\" ', j, '0a.tif', ' -r bilinear -te -180 -90 180 90 -tr ', 1/5,' ', 1/5, sep=""))  
  #}
}
